...
func bindRuntimeFlags(fs *flag.FlagSet, cfg *node.Config) {
	...
	fs.StringVar(&cfg.RuntimeActivationEncoding, "runtime-activation-encoding", cfg.RuntimeActivationEncoding, "inter-stage activation encoding: f32 or f16")
	fs.StringVar(&cfg.RuntimeKVCacheType, "runtime-kv-cache-type", cfg.RuntimeKVCacheType, "llama.cpp KV cache type: f16 or q8_0")
	...
	fs.IntVar(&cfg.RuntimeUBatchSize, "runtime-ubatch-size", cfg.RuntimeUBatchSize, "llama.cpp physical micro-batch size")
	...
	fs.IntVar(&cfg.RuntimeParallelSessions, "runtime-parallel-sessions", cfg.RuntimeParallelSessions, "resident llama.cpp session capacity when continuous batching is enabled")
	fs.IntVar(&cfg.RuntimeDecodeBatchSize, "runtime-decode-batch-size", cfg.RuntimeDecodeBatchSize, "continuous decode batch size; one disables batching")
	fs.StringVar(&cfg.RuntimeSpeculativeDraft, "runtime-speculative-draft", cfg.RuntimeSpeculativeDraft, "speculative draft strategy: none or prompt_lookup")
	fs.IntVar(&cfg.RuntimeSpeculativeMax, "runtime-speculative-max-tokens", cfg.RuntimeSpeculativeMax, "maximum speculative draft tokens per target pass")
