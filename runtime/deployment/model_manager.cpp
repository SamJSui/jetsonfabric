    ...
    Impl(
        ...
        if (!config.start_idle) {
            insert_active(
                ...
                engine_factory.create_engine(config)
            );
        }
    }
...
DeploymentStatus ModelManager::deployment_status() const {
    return impl_->deployment_status();
}

DeploymentStatus ModelManager::deployment_status(
    const DeploymentIdentity& identity
) const {
...
LoadDeploymentResult ModelManager::load_resident_deployment(
    ...
    return impl_->load_resident_deployment(
        std::move(node_name),
        std::move(identity),
        assignment,
        std::move(build_engine_parts)
    );
}

ActivateDeploymentResult ModelManager::activate_resident_deployment(
    const DeploymentIdentity& expected_identity
) {
    return impl_->activate_resident_deployment(expected_identity);
}

DrainDeploymentResult ModelManager::drain_resident_deployment(
    const DeploymentIdentity& expected_identity
) {
    return impl_->drain_resident_deployment(expected_identity);
}

UnloadDeploymentResult ModelManager::unload_resident_deployment(
    const DeploymentIdentity& expected_identity
) {
